        ...
        <div class="form-group isPicker {{ $errors->has('erp') ? ' is-invalid' : '' }}">
            <select class="selectpicker @if (old('erp') !='') started @endif " name="erp" id="erp" title="{{ __('app.lead.erp') }}" data-size="5">
                @foreach(App\Models\Erp::all() as $key => $value)
                    <option value='{{$key}}' @if (old('erp') == $key)selected @endif data-content="<div class='hideHint'>{{ __('app.lead.erp') }} </div><div class='colored'> {{ $value['name'] }}</div>">
                        {{ $value['name'] }}
                    </option>
                @endforeach
            ...
            @include('app.lead.components.error-field', ['name' => 'erp'])
        </div>
    </div>
    <div class="col-sm-6 col-md-6">
        <div class="form-group hasDW @if (old('erp') != -1) {{ 'disabled' }} @endif ">
            ...
            @include('app.lead.components.error-field', ['name' => 'erp_other'])
        </div>
    </div>
</div>
